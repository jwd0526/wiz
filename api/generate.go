...
// Configuration
func newConfig(req GenerateRequest) (Config, error) {
...
// Game Generation
func generateGame(config Config) (Game, error) {
	...
	game.Walls = placeWalls(game.Board, game.Path, config.Walls)
...
func placeGameNodes(g Game, c Config) Game {
	board := g.Board
	path := g.Path
	pathLen := len(path)
	...
	return Game{Board: board, Path: path, Walls: g.Walls}
}
...
func checkWallBlocksPath(wall Wall, path []Node) bool {
...
func generateWall(x, y int, horizontal bool, size int, wouldBlockPath func(Wall) bool) *Wall {
	if horizontal {
		if y >= size-1 {
			return nil
		}
		...
		return &Wall{X1: x, Y1: y, X2: x + length - 1, Y2: y, Horizontal: true}
	} else {
		if x >= size-1 {
			return nil
		}
		maxLength := calculateMaxLength(x, y, horizontal, size, wouldBlockPath)
		length := calculateWallLength(maxLength)
		return &Wall{X1: x, Y1: y, X2: x, Y2: y + length - 1, Horizontal: false}
	}
}

func calculateMaxLength(x, y int, horizontal bool, size int, wouldBlockPath func(Wall) bool) int {
	maxLength := 1
	if horizontal {
...
func hasOverlap(wall Wall, walls []Wall) bool {
